...
import { PLAYER_POSITION_LABEL, SETTING_ATTACK_ZONE_LABEL, } from "@/lib/volley-store";
/* --------------------------- helpers --------------------------- */
...
function computeStats(events, playerId) {
    let points = 0, attackKills = 0, attackAttempts = 0, attackErrors = 0, blocks = 0, aces = 0, recTotal = 0, recPositive = 0;
    for (const ev of events) {
        if ("scoringSide" in ev && ev.playerId === playerId) {
            const pe = ev;
            ...
            if (ev.kind === "reception" && ev.playerId === playerId) {
                const r = ev;
                recTotal++;
                if (r.rating === "double_positive" || r.rating === "positive")
                    recPositive++;
            }
...
function classifyHighlight(ev, playerId) {
    if ("scoringSide" in ev && ev.playerId === playerId) {
        const pe = ev;
        const isPoint = pe.playerSide === pe.scoringSide;
        if (pe.type === "ace" && isPoint)
            return "ace";
        if (pe.type === "block" && isPoint)
            return "block";
        if (isPoint && (pe.type === "attack" || pe.type === "counter_attack" || pe.type === "rotation_attack"))
            return "attack";
        if (!isPoint)
            return "error";
    }
...
function describeLastAction(events, playerId) {
    for (let i = events.length - 1; i >= 0; i--) {
        ...
        if ("kind" in ev) {
            ...
            if (ev.kind === "attackAttempt" && ev.playerId === playerId) {
                const zone = ev.attackZone ? ` · ${SETTING_ATTACK_ZONE_LABEL[("z" + ev.attackZone)] ?? "Z" + ev.attackZone}` : "";
...
function computeBadges(events, playerId, teamPlayerIds) {
    const perPlayer = new Map();
    for (const pid of teamPlayerIds)
        perPlayer.set(pid, computeStats(events, pid));
    const me = perPlayer.get(playerId);
    if (!me)
        return [];
    const isLeader = (fn, min = 1) => {
        const val = fn(me);
        if (val < min)
            return false;
    ...
    const badges = [];
    if (isLeader((s) => s.points, 3))
        badges.push("scorer");
    if (isLeader((s) => s.aces, 2))
        badges.push("aces");
    if (isLeader((s) => s.blocks, 2))
        badges.push("block");
    if (isLeader((s) => (s.recTotal >= 4 ? s.effRec : -1), 60))
        badges.push("reception");
    // MVP: kills + aces + blocks combined leader with meaningful volume
    if (isLeader((s) => s.attackKills + s.aces + s.blocks, 6))
        badges.unshift("mvp");
    return badges.slice(0, 3);
}
/* --------------------------- contexto del partido --------------------------- */
function buildContext(events, playerId, currentSet, playerSide) {
    const notes = [];
    ...
    if (streak >= 2)
        notes.push(`${streak} puntos consecutivos sin error`);
    // Set actual
    const setStats = computeStats(events.filter((ev) => ("setNumber" in ev ? ev.setNumber === currentSet : false)), playerId);
    ...
    if (touched >= 2)
        notes.push(`Participó en ${touched} de los últimos 3 rallies`);
    ...
    if (zoneCount.size > 0) {
        ...
        if (top[1] >= 2)
            notes.push(`Zona frecuente: ${top[0]}`);
    }
    void playerSide;
    return notes.slice(0, 3);
}
/* --------------------------- tone helpers --------------------------- */
function effTone(eff, samples) {
    if (samples < 3)
...
export function CourtPlayerBadge({ player, team, match, isServer, isLibero, isReceiverHighlight, active, dimmed, onClick, className, style, }) {
    ...
    useEffect(() => {
        ...
        if (lastRelevant.id === lastIdRef.current)
            return;
        lastIdRef.current = lastRelevant.id;
    ...
    const isLastActor = useMemo(() => {
        if (!newestEventId || !lastRelevant)
            return false;
        return lastRelevant.id === newestEventId;
    }, [newestEventId, lastRelevant]);
    return (<HoverCard open={isOpen} onOpenChange={(o) => setHover(o)} openDelay={120} closeDelay={150}>
      <HoverCardTrigger asChild>

        <button type="button" ref={triggerRef} onClick={onClick} onContextMenu={handleContext} title={`#${player.number} ${player.name}`} className={[
          ...
          <span className="absolute inset-0 rounded-full overflow-hidden flex items-center justify-center" style={{
            ...
            {player.photoUrl && imgOk ? (<img src={player.photoUrl} alt={player.name} loading="lazy" onError={() => setImgOk(false)} className="w-full h-full object-cover" draggable={false}/>) : (<span className="scoreboard-digit font-black leading-none text-[clamp(1rem,4vw,2.2rem)]" style={{
          ...
          <span className="absolute -bottom-1 -right-1 min-w-[45%] h-[45%] px-1 rounded-full flex items-center justify-center scoreboard-digit font-black text-[clamp(0.7rem,2.2vw,1.2rem)] leading-none tabular-nums shadow-md" style={{
            background: "#111827",
            color: "#ffffff",
          ...
          {isServer && (<span className="absolute -top-1 left-1/2 -translate-x-1/2 z-10 flex h-3 w-3 items-center justify-center">
          ...
          {isLibero && (<span className="absolute -top-1.5 -left-1.5 z-10 grid place-items-center size-5 rounded-full text-[10px] font-black uppercase tracking-widest text-white shadow-md ring-2 ring-white" style={{ background: team.color }} aria-label="Líbero">
              L
            </span>)}

          {isReceiverHighlight && (<span className="absolute -inset-1 rounded-full ring-2 ring-yellow-300 animate-pulse pointer-events-none"/>)}
      ...
      <HoverCardContent ref={contentRef} side="top" sideOffset={12} align="center" collisionPadding={16} avoidCollisions sticky="always" className="w-[240px] p-0 overflow-hidden border-border/70 bg-popover/95 backdrop-blur-sm shadow-2xl ring-1 ring-black/20 data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=open]:zoom-in-95">

        <PlayerTooltipContent player={player} team={team} match={match} initials={initials} imgOk={imgOk} isLastActor={isLastActor} active={active} pinned={pinned} onTogglePin={() => setPinned((p) => !p)}/>
      </HoverCardContent>
    </HoverCard>);
}
/* --------------------------- tooltip content --------------------------- */
function PlayerTooltipContent({ player, team, match, initials, imgOk, isLastActor, active, pinned, onTogglePin, }) {
    ...
    const badges = useMemo(() => computeBadges(match.events, player.id, teamPlayerIds), [match.events, player.id, teamPlayerIds]);
    const playerSide = team.id === match.teamAId ? "A" : "B";
    const context = useMemo(() => buildContext(match.events, player.id, match.currentSet, playerSide), [match.events, player.id, match.currentSet, playerSide]);
    const rows = [];
    ...
    if (stats.points)
        rows.push({ key: "pts", icon: "🔥", label: "Puntos", value: String(stats.points) });
    if (stats.aces)
        rows.push({ key: "ace", icon: "⚡", label: "Aces", value: String(stats.aces) });
    if (stats.blocks)
        rows.push({ key: "blk", icon: "🛡", label: "Bloqueos", value: String(stats.blocks) });
    if (stats.recTotal)
        rows.push({ key: "rec", icon: "📥", label: "Recepción", value: `${stats.recPositive}/${stats.recTotal} · ${stats.effRec}%`, tone: effTone(stats.effRec, stats.recTotal) });
    ...
    return (<div className="text-[12px]">
      {/* Header */}
      <div className="flex items-center gap-2.5 px-3 py-2 border-b border-border/60" style={{ background: `${team.color}1a` }}>
        <div className="size-10 rounded-full overflow-hidden shrink-0 flex items-center justify-center" style={{ background: team.color }}>
          {player.photoUrl && imgOk ? (<img src={player.photoUrl} alt={player.name} className="w-full h-full object-cover"/>) : (<span className="text-white font-black text-sm">{initials}</span>)}
        </div>
        ...
        <button type="button" onClick={onTogglePin} className={`shrink-0 size-6 grid place-items-center rounded-md border border-border/60 transition-colors ${pinned ? "bg-primary text-primary-foreground border-primary" : "text-muted-foreground hover:text-foreground"}`} aria-label={pinned ? "Desfijar" : "Fijar"} title={pinned ? "Desfijar" : "Fijar (o clic derecho)"}>
          <Pin className="size-3"/>
        </button>
      </div>

      {/* Status + badges */}
      {(statusPill || badges.length > 0) && (<div className="flex flex-wrap gap-1 px-3 pt-2">
          {statusPill && (<span className={`px-1.5 py-0.5 rounded-full border text-[9px] font-bold uppercase tracking-wide ${statusPill.cls}`}>
              {statusPill.text}
            </span>)}
          {badges.map((b) => {
                const meta = BADGE_META[b];
                return (<span key={b} className={`px-1.5 py-0.5 rounded-full border text-[9px] font-bold uppercase tracking-wide ${meta.className}`} title={meta.label}>
      ...
      {rows.length > 0 ? (<div className="px-3 py-2 grid grid-cols-1 gap-1">
          {rows.map((r) => (<div key={r.key} className="flex items-center justify-between gap-2">
        ...
        </div>) : (<div className="px-3 py-2 text-[11px] text-muted-foreground italic">Sin acciones registradas aún.</div>)}

      {/* Última acción */}
      {lastAction && (<div className="px-3 py-2 border-t border-border/60">
          <div className="text-[9px] uppercase tracking-widest text-muted-foreground font-bold mb-0.5">Última acción</div>
          <div className={`text-[12px] font-bold ${lastAction.tone === "good"
                ? "text-emerald-400"
                : lastAction.tone === "bad"
                    ? "text-rose-400"
                    : "text-foreground"}`}>
      ...
      {/* Contexto */}
      {context.length > 0 && (<div className="px-3 py-2 border-t border-border/60 space-y-0.5">
          <div className="text-[9px] uppercase tracking-widest text-muted-foreground font-bold mb-0.5">Contexto</div>
          {context.map((c, i) => (<div key={i} className="text-[11px] text-foreground/90 leading-snug">• {c}</div>))}
        </div>)}

      {/* Ver perfil */}
      <Link to="/jugadora/$id" params={{ id: player.id }} className="flex items-center justify-between gap-2 px-3 py-2 border-t border-border/60 text-[11px] font-semibold text-primary hover:bg-primary/10 transition-colors">
