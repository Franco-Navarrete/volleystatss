...
function drawH1(ctx: RenderCtx, title: string) {
  ...
  ensureSpace(ctx, 40);
  ctx.toc.push({ title, page: ctx.doc.internal.getCurrentPageInfo().pageNumber, level: 1 });
...
function renderVisualSummary(ctx: RenderCtx, a: MatchAnalysis, summaryMd?: string) {
  ...
  hairline(ctx.doc, ctx.margin, ctx.y, ctx.margin + 40, ctx.y, C.navy, 0.6);
  ctx.y += 8;
  ctx.toc.push({ title: "Resumen Ejecutivo (hero)", page: ctx.doc.internal.getCurrentPageInfo().pageNumber, level: 1 });
